        ...
        imageFeed.map((item, index) => ( 
          <div key={item.index} className="w-full sm:w-1/2 lg:w-1/3 p-4">
            <div className="rounded-lg shadow-md bg-neutral-900 text-white">
              ...
              <span className='flex justify-end pr-4 pt-4 cursor-pointer' onClick={() => deletePost(item)}><DeleteIcon sx={{color: 'grey', "&:hover": { color: "white" }}} /></span>
              <div className="p-4">
                  <div className='flex justify-between'>
                      <h2 className="text-lg font-medium">{item.type}</h2>
                      <h2 className="text-md font-medium text-amber-200">{item.location}</h2>
                  </div>
              <div className='flex justify-between'>
